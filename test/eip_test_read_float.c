...
** $Header: /home/cjm/cvs/eip/test/eip_test_read_float.c,v 1.3 2011-01-12 14:12:26 cjm Exp $
*/
 ...
 */
static char rcsid[] = "$Id: eip_test_read_float.c,v 1.3 2011-01-12 14:12:26 cjm Exp $";
...
int main(int argc, char *argv[])
{
	...
	/* open interface */
	if(!EIP_Session_Handle_Create("eip_test_read_float",NULL,&handle))
	{
		EIP_General_Error();
		return 4;
	}
	/* open a session to a Micrologix 1100 backplane 1, slot 0 @ Hostname */
	if(!EIP_Session_Open("eip_test_read_float",NULL,Hostname,1,0,PLC_TYPE_MICROLOGIX1100,handle))
	{
		EIP_General_Error();
		EIP_Session_Handle_Destroy("eip_test_read_float",NULL,&handle);
		return 4;
	}
	/* read the float */
	if(!EIP_Read_Float("eip_test_read_float",NULL,handle,PLC_Address,&value))
	{
		EIP_General_Error();
		EIP_Session_Handle_Destroy("eip_test_read_float",NULL,&handle);
		return 4;
	...
	/* close */
	if(!EIP_Session_Close("eip_test_read_float",NULL,handle))
	{
		EIP_General_Error();
		EIP_Session_Handle_Destroy("eip_test_read_float",NULL,&handle);
		return 4;
	}
	if(!EIP_Session_Handle_Destroy("eip_test_read_float",NULL,&handle))
	{
		EIP_General_Error();
		return 4;
	}
